...
"use strict";$(document).ready(function(){var e={clientId:"13VQ4NTCS0U33XREVAZLYWOPN3NXDNLHOGKL1SJNRAPZL0FZ",clientSecret:"EUJ5KAQEQRFUFLIDNS21ZT4KQ3J5RX2XVAGY2YBOT21HGUVX"},t=new google.maps.Geocoder,o=function(e,t,o,i,s,n){this.placeName=ko.observable(e),this.contact=ko.observable(t),this.location=ko.observable(o),this.category=ko.observable(i),this.icon=ko.observable(s),this.opened=ko.observable(n)},i=function(i,s){var n=this;this.location=i,this.marker=s;var a="Fetching address...";this.geocode=null,this.country=ko.observable(""),this.city=ko.observable(""),this.street=ko.observable(""),this.building=ko.observable(""),this.address=ko.observable(a),this.topPicks=ko.observableArray([]),google.maps.event.addListener(s,"click",function(){window.viewModel.selectLocation(n),$("#placesModal").modal("show"),map.panTo(s.location)}),t.geocode({latLng:i},function(e,t){if(t==google.maps.GeocoderStatus.OK){n.address(e[0].formatted_address),n.geocode=e;for(var o=e[0],i=0,r=o.address_components.length;r>i;i++){var c=o.address_components[i];c.types.indexOf("street_number")>-1?n.building(c.long_name):c.types.indexOf("route")>-1?n.street(c.long_name):c.types.indexOf("locality")>-1?n.city(c.long_name):c.types.indexOf("country")>-1&&n.country(c.long_name)}}else n.address()===a&&n.address("Failed to fetch location address");s.setTitle(n.address())});var r=function(){var t=0;$.ajax("https://api.foursquare.com/v2/venues/explore?"+$.param({ll:i.lat()+","+i.lng(),client_id:e.clientId,client_secret:e.clientSecret,section:"topPicks",v:"20150618",limit:8,radius:200})).success(function(e){n.topPicks([]);for(var t=0,i=e.response.groups[0].items.length;i>t;t++){var s=e.response.groups[0].items[t].venue,a="---";s.location&&s.location.formattedAddress&&(a=s.location.formattedAddress);var r="---";s.categories&&s.categories[0]&&s.categories[0].name&&(r=s.categories[0].name);var c="";s.categories&&s.categories[0]&&s.categories[0].icon&&(c=s.categories[0].icon.prefix+"64"+s.categories[0].icon.suffix);var l="";s.hours&&s.hours.status&&(l=s.hours.status),n.topPicks.push(new o(s.name||"Unknown place",s.contact.formattedPhone||"No contact specified",a,r,c,l))}}).fail(function(){2>t?(t+=1,r()):alert("Ups. Something went wrong. We could have apologised, but that is either your connection or forsquare failing - not our beautiful website.")})};r()},s=function(){var e=this;this.filterCountry=ko.observable(""),this.filterCity=ko.observable(""),this.filterStreet=ko.observable(""),this.selectedLocation=ko.observable(),this.locations=ko.observableArray([]),this.selectLocation=function(t){e.selectedLocation()&&e.selectedLocation().marker.setIcon("http://maps.google.com/mapfiles/ms/icons/red-dot.png"),t.marker.setIcon("http://maps.google.com/mapfiles/ms/icons/blue-dot.png"),map.panTo(t.marker.position),e.selectedLocation(t)},this.i=0,this.startBounce=function(e){e.marker.setAnimation(google.maps.Animation.BOUNCE)},this.stopBounce=function(e){e.marker.setAnimation(null)},this.filterLocations=function(){if(this.filterCountry()||this.filterCity()||this.filterStreet()){for(var e=this.locations(),t=e.length-1;t>=0;t--){var o=e[t];this.filterCountry()&&-1===o.country().toLowerCase().indexOf(this.filterCountry().toLowerCase())?this.removeLocation(e[t]):this.filterCity()&&-1===o.city().toLowerCase().indexOf(this.filterCity().toLowerCase())?this.removeLocation(e[t]):this.filterStreet()&&-1===o.street().toLowerCase().indexOf(this.filterStreet().toLowerCase())&&this.removeLocation(e[t])}this.locations(e)}},this.addLocation=function(e,t){this.locations.push(new i(e,t))},this.removeLocation=function(e){for(var t=this.locations(),o=t.length-1;o>=0;o--)t[o]===e&&(e.marker.setMap(null),t.splice(o,1));this.locations(t)},this.clearLocations=function(){for(var e=this.locations().length-1;e>=0;e--)this.removeLocation(this.locations()[e])}};window.viewModel=new s,ko.applyBindings(window.viewModel),$("#menu-toggle").click(function(e){e.preventDefault(),$("#wrapper").toggleClass("toggled"),setTimeout(function(){google.maps.event.trigger(map,"resize")},1e3)}),Math.max(document.documentElement.clientWidth,window.innerWidth||0)>=768&&$("#wrapper").toggleClass("toggled")});
...
"use strict";function createMap(){var e=[{placeName:"New York, Metropolitan Opera House",lat:40.7731527,lng:-73.98493630000002}],o=e[Math.floor(Math.random()*e.length)],t={disableDefaultUI:!0,maxZoom:25,zoom:15,mapTypeId:google.maps.MapTypeId.ROADMAP,center:{lat:o.lat,lng:o.lng},disableDoubleClickZoom:!0};map=new google.maps.Map(document.querySelector("#map"),t);var a=null;google.maps.event.addListener(map,"click",function(e){a=setTimeout(function(){},200)});var n=function(e){return new google.maps.Marker({position:e,map:map,title:"NewLocation",icon:"http://maps.google.com/mapfiles/ms/icons/red-dot.png"})};google.maps.event.addListener(map,"dblclick",function(e){clearTimeout(a),window.viewModel.addLocation(e.latLng,n(e.latLng)),e.stop()});var l=document.getElementById("place-filter"),i=document.getElementById("menu-toggle");l.style.maxWidth="200px",i.style.width="auto",map.controls[google.maps.ControlPosition.TOP_LEFT].push(i),map.controls[google.maps.ControlPosition.TOP_LEFT].push(l);var m=new google.maps.places.Autocomplete(l);m.bindTo("bounds",map),google.maps.event.addListener(m,"place_changed",function(){var e=m.getPlace();e.geometry&&(e.geometry.viewport?map.fitBounds(e.geometry.viewport):(map.setCenter(e.geometry.location),window.viewModel.addLocation(e.geometry.location,n(e.geometry.location))))})}function getLocation(e){var o=new google.maps.places.PlacesService(map);o.textSearch({query:e},function(e,o){o==google.maps.places.PlacesServiceStatus.OK&&(console.log(e[0].geometry.location.lat()),console.log(e[0].geometry.location.lng()))})}var map=null,mapBounds=null;window.addEventListener("load",createMap),window.addEventListener("resize",function(e){map.fitBounds(mapBounds)});